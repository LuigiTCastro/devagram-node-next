    ...
    try {
        if(req?.method === 'PUT') {
            ...
            if(userIndexOnLike != -1) {
                ...
                await PublicationModel.findByIdAndUpdate({ id : publication._id }, publication);
                ...
                await PublicationModel.findByIdAndUpdate({ id : publication._id }, publication) // finds the publication and updates with the new data.
        ...
        console.log(e);
        return res.status(500).json({ error: 'There was an error liking/disliking the photo.' });
    }
}
